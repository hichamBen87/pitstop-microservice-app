  ...
  try {
    ...
    next(
      createError(
        ...
        "An unexpected error in our end while getting vehicles list",
      ),
    );
  }
};
...
exports.allJobsDay = async (req, res, next) => {
  try {
    ...
    next(
      createError(
        createError.InternalServerError,
        "An unexpected error in our end while getting jobs list",
      ),
    );
  }
};
...
exports.jobDetails = async (req, res, next) => {
  try {
    ...
    next(
      createError(
        createError.InternalServerError,
        "An unexpected error in our end while getting job details",
      ),
    );
  }
};
...
exports.scheduleJob = async (req, res, next) => {
  try {
    let { date, startTime, endTime, description, vehicle, status } = req.body;
    ...
    if (maintenance) {
      res
        .status(resCode.OK)
        .send(
          `Maintenance job of vehicle ${vehicle} has been scheduled successfully`,
        );
    }
  } catch (err) {
    next(
      createError(
        createError.InternalServerError,
        "An unexpected error in our end while scheduling a maintenance job list",
      ),
    );
  }
};
  ...
  try {
    ...
    if (deleted == 1) {
      res
        .status(resCode.OK)
        .send(
          `Maintenance job with license ${id} has been deleted successfully`,
        );
    }
  } catch (err) {
    next(
      createError(
        createError.InternalServerError,
        "An unexpected error in our end while deleting a maintenance job",
      ),
    );
  }
};
...
exports.completeJob = async (req, res, next) => {
  try {
    let id = req.params.id;
    let updated = await db.MaintenanceJob.update(
      { status: "completed" },
      {
        where: {
          id,
    ...
    if (updated[0] == 1) {
      res
        .status(resCode.OK)
        .send(
          `Maintenance job with license ${id} has been completed successfully`,
        );
    }
  } catch (err) {
    next(
      createError(
        createError.InternalServerError,
        "An unexpected error in our end while completing a maintenance job",
      ),
    );
  }
};
