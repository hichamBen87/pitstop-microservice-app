  ...
  try {
    ...
    next(
      createError(
        ...
        "An unexpected error in our end while getting customers list",
      ),
    );
  }
};
...
exports.registerVehicle = async (req, res, next) => {
  try {
    let { license, brand, type, owner } = req.body;
    ...
    if (vehicle) {
      ...
      res
        .status(resCode.OK)
        .send(`Vehicle ${license} has been added successfully`);
    }
  } catch (err) {
    next(
      createError(
        createError.InternalServerError,
        "An unexpected error in our end while registering a new vehicle",
      ),
    );
  }
};
